...
- **全平移精确枚举（双路径，无任何投影/阈值剪枝）**：对每种姿态，精确覆盖全部 `(2N-1)^2` 个整数平移，
  二维最优平移在任一维投影上都可能不是最优，因此不得只枚举行/列方向各自最佳偏移的笛卡尔积。
  - 稀疏路径（缺陷点对）：对每个 (参考点, 姿态变换后的复检点) 对，平移 `(rr-pr, cc-pc)` 恰好命中一次，
    用位移直方图累计每个整数平移的重合数，复杂度 `O(Kref·Krec)`。
  - 稠密路径（自实现二维 FFT）：当 `Kref·Krec ≥ 4·L²`（L 为不小于 2N-1 的最小 2 的幂）时启用，
    一次二维相关算出全部 `(2N-1)^2` 个平移的重合数，复杂度 `O(N² log N)`——
    稠密满尺寸（512×512 全 1）实测单次审计约 0.7 s。
...
- 正确性由 `backend/correlate_test.go` 与 `backend/scenario_test.go` 保证：多尺寸多密度随机输入、
  阈值两侧输入及题述稀疏四组并列场景下，搜索结果（最大重合、并列数量、规范解）与整数穷举完全一致。
...
2. 经前端 nginx 代理的端到端用例：确定性姿态+平移、镜像姿态、移出画布计数、
   题述 16×16 稀疏点阵四组并列最优（独立整数穷举核对四组解、最大重合 4、并列数 4、
   规范解 rot90/(4,0)、四个重合点的红蓝叠加证据）、全 0、稀疏唯一最优、
